...
function renderDifferencesGrid() {
    ...
    return `
        <div style="background: white; padding: 2.5rem; border-radius: 16px; margin: 2.5rem 0; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h3 style="margin: 0 0 2rem 0; color: #8b5cf6; font-size: 1.5rem; font-weight: 700;"><i class="fas fa-star"></i> Diferencas-Chave</h3>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 1.75rem;">
                ${differences.map(diff => `
                    <div style="background: linear-gradient(135deg, #f9fafb 0%, white 100%); padding: 2rem; border-radius: 12px; border: 2px solid #e5e7eb; transition: all 0.3s; cursor: pointer;" onmouseover="this.style.transform='translateY(-4px)'; this.style.boxShadow='0 12px 30px rgba(0,0,0,0.15)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 12px rgba(0,0,0,0.08)'">
                        <div style="font-size: 2rem; margin-bottom: 1.25rem; font-weight: bold;">${diff.icon}</div>
                        <h4 style="color: #333; margin-bottom: 1.25rem; font-size: 1.1rem; font-weight: 700;">${diff.title}</h4>
                        <div style="display: flex; flex-direction: column; gap: 1rem;">
                            <div style="padding: 1rem 1.25rem; border-radius: 8px; font-size: 0.95rem; border-left: 4px solid #ea580c; background: rgba(234, 88, 12, 0.08); color: #92400e;">
                                <strong>Mitose:</strong> ${diff.mitose}
                            </div>
                            <div style="padding: 1rem 1.25rem; border-radius: 8px; font-size: 0.95rem; border-left: 4px solid #059669; background: rgba(5, 150, 105, 0.08); color: #047857;">
...
function renderTimeline() {
    return `
        <div style="background: white; padding: 2.5rem; border-radius: 16px; margin: 2.5rem 0; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
            <h3 style="margin: 0 0 2.5rem 0; color: #667eea; font-size: 1.5rem; font-weight: 700;"><i class="fas fa-stream"></i> Cronograma das Fases</h3>
            <div style="overflow-x: auto; padding: 2rem 0;">
                <div style="display: flex; justify-content: space-around; gap: 2rem; min-width: 100%; position: relative; padding: 0 1rem;">
                    ${[0, 1, 2, 3].map(i => `
                        <button type="button" onclick="window.ComparisonApp.setPhase(${i})" style="flex: 1; text-align: center; cursor: pointer; position: relative; z-index: 1; transition: all 0.3s; border: none; background: none; padding: 0;">
                            <div style="width: 50px; height: 50px; border-radius: 50%; background: ${i === AppState.currentPhase ? 'linear-gradient(135deg, #667eea 0%, #8b5cf6 100%)' : 'white'}; border: 3px solid ${i === AppState.currentPhase ? 'transparent' : '#ddd'}; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.25rem; transition: all 0.3s; box-shadow: ${i === AppState.currentPhase ? '0 0 20px rgba(102, 126, 234, 0.4)' : '0 2px 8px rgba(0,0,0,0.1)'}; color: ${i === AppState.currentPhase ? 'white' : '#999'}; font-weight: 700;"></div>
                            <div style="font-weight: 600; color: ${i === AppState.currentPhase ? '#667eea' : '#666'}; font-size: 0.95rem;">${PHASES_EXPLANATION[i].name}</div>
                        </button>
                    `).join('')}
                </div>
            </div>
